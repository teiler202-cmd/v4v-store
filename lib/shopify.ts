...
import { inContext, shopifyFetch, type StorefrontLanguage } from '@/lib/shopify-config';
...
export async function getProducts(language?: StorefrontLanguage) {
  ...
  const data = await shopifyFetch<any>(query, undefined, { revalidate: 60, language });
 ...
 */
export async function getProduct(handle: string, language?: StorefrontLanguage) {
  const query = `
    query getProduct($handle: String!)${inContext(language)} {
      product(handle: $handle) {
        ...
        #
        # availableForSale 은 빠뜨리면 안 됩니다 — 없으면 isVariantAvailable 이
        # 'undefined !== false' 로 판정해 품절 상품에도 'Add to Bag'을 띄웁니다.
        # 손님은 담을 수 있고, 결제창에 가서야 살 수 없다는 걸 알게 됩니다.
        variants(first: 250) {
          edges {
            node {
              id
  ...
  const data = await shopifyFetch<any>(query, variables, { revalidate: 60, language });
